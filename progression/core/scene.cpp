...
namespace Progression {
    ...
    Scene* Scene::Load(const std::string& filename) {
        ...
        while (std::getline(in, line)) {
            ...
            if (line == "Resource-file") {
                ...
                std::cout << "parsing bg color" << std::endl;
                std::getline(in, line);
                std::stringstream ss(line);
                glm::vec4 c;
                ss >> c;
                scene->SetBackgroundColor(c);
            } else if (line == "RTSkybox") {
                std::string name = "";
                line = " ";
                while (line != "" && !in.eof()) {
                    std::getline(in, line);
                    ...
                    ss >> first;
                    if (first == "right") {
                        ss >> scene->rtskybox.right;
                    } else if (first == "left") {
                        ss >> scene->rtskybox.left;
                    } else if (first == "top") {
                        ss >> scene->rtskybox.top;
                    } else if (first == "bottom") {
                        ss >> scene->rtskybox.bottom;
                    } else if (first == "front") {
                        ss >> scene->rtskybox.front;
                    } else if (first == "back") {
                        ss >> scene->rtskybox.back;
                    }
                }
            }
        }
    ...
    void Scene::ParseGameObject(Scene* scene, std::ifstream& in) {
        ...
        while (line != "" && !in.eof()) {
            ...
            } else if (first == "rtinfo") {
                auto c = new RTInfo(obj);
                while (line != "" && !in.eof()) {
                    std::getline(in, line);
